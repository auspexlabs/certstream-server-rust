...
async fn discover_and_spawn(
    ...
    // Splice in configured static logs by log_id when provided. A configured
    // static log with the same log_id as a discovered log replaces it only if
    // non-replaceable identity fields still agree.
